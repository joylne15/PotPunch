...
export default function Login() {
  ...
  function handleLogin() {
    ...
    // Accept any email and password for now; backend auth will replace this.
    if (role === "admin") {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: "#F0F4F8" }}>
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 w-full max-w-md overflow-hidden">
        <div className="text-white text-center py-8 px-6" style={{ background: "linear-gradient(135deg, #1D9E75 0%, #0F6E56 100%)" }}>
        ...
        <div className="p-6">
          <div className="grid grid-cols-2 gap-3 mb-6">
            <button
              ...
              className={`border rounded-xl py-4 text-sm font-medium transition-all hover:-translate-y-0.5 ${
                role === "admin"
                  ? "border-green-500 bg-green-50 text-green-700"
                  : "border-gray-200 text-gray-500 hover:bg-gray-50"
              }`}
            >
              <div className="text-2xl mb-1">{"\u{1F6E1}\uFE0F"}</div>
              <div>Admin</div>
          ...
          <button
            onClick={handleLogin}
            className="w-full text-white rounded-xl py-3 text-sm font-semibold transition-all hover:shadow-md"
